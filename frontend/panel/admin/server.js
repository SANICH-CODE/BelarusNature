        ...
        fetch('https://sanich-code.github.io/BN/users.json')
            ...
            .then(data => {
                ...
                const usersArray = Object.keys(data).map(userId => ({
                    ...
                    RegDate: data[userId].RegDate,
                    RegTime: data[userId].RegTime
                }));
    
                if (usersArray.length === 0) {
                    content.innerHTML = '<div class="alert alert-warning">Нет данных для отображения!</div>';
                    return;
                }
                ...
                ['ID', 'Username', 'Дата регистрации', 'Время регистрации'].forEach(headerText => {
                ...
                usersArray.forEach(user => {
                    ...
                    ['id', 'username', 'RegDate', 'RegTime'].forEach(key => {
...
content.innerHTML = `
    <div class="container">
        ...
        <div class="d-grid gap-2" id="buttonContainer">
            ...
            <button class="btn btn-primary btn-lg backend-btn" data-url="/backend/root">📜 Получить доступ</button>
            <button class="btn btn-danger btn-lg backend-btn" data-url="/backend/restart">🔄 Перезагрузить сервер 🚬</button>
            <button class="btn btn-dark btn-lg backend-btn" data-url="/backend/shutdown">⛔ Выключить сервер</button>
        </div>
